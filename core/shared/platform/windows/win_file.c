...
// Get the filepath for a handle. The size of the buffer should in terms of
// wchar.
static __wasi_errno_t
...
read_next_dir_entry(os_dir_stream dir_stream, FILE_ID_BOTH_DIR_INFO **out_entry)
{
    ...
    int ret = WideCharToMultiByte(CP_UTF8, 0, current_info->FileName,
                        current_info->FileNameLength
                            / (sizeof(wchar_t) / sizeof(char)),
                        dir_stream->current_entry_name,
                        sizeof(dir_stream->current_entry_name), NULL, NULL);
    if (ret == 0)
        return convert_windows_error_code(GetLastError());
...
get_disk_file_information(HANDLE handle, __wasi_filestat_t *buf)
{
    ...
    do {
        ...
        buf->st_ino = (__wasi_inode_t)(file_id_both_dir_info->FileId.QuadPart);
        buf->st_atim = convert_filetime(
            (LPFILETIME)&file_id_both_dir_info->LastAccessTime.QuadPart);
        buf->st_mtim = convert_filetime(
            (LPFILETIME)&file_id_both_dir_info->LastWriteTime.QuadPart);
        buf->st_ctim = convert_filetime(
...
get_disk_file_information(HANDLE handle, __wasi_filestat_t *buf)
{
    ...
    buf->st_atim =
        convert_filetime((LPFILETIME)&file_basic_info.LastAccessTime.QuadPart);
    buf->st_mtim =
        convert_filetime((LPFILETIME)&file_basic_info.LastWriteTime.QuadPart);
    buf->st_ctim =
        convert_filetime((LPFILETIME)&file_basic_info.ChangeTime.QuadPart);
          ...
          __wasi_lookupflags_t lookup_flags)
{
    ...
    CHECK_VALID_PATH(from_path);
    CHECK_VALID_PATH(to_path);
            ...
            os_file_handle new_handle, const char *new_path)
{
    ...
    CHECK_VALID_PATH(old_path);
    CHECK_VALID_PATH(new_path);
...
os_is_handle_valid(os_file_handle *handle)
{
    ...
    if ((*handle) == NULL
        || ((*handle)->type == windows_handle_type_socket
            && (*handle)->raw.socket == INVALID_SOCKET)
        || ((*handle)->type == windows_handle_type_file
            && (*handle)->raw.handle == INVALID_HANDLE_VALUE)) {
        return false;
    }
