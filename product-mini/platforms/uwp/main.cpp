...
char global_heap_buf[WASM_GLOBAL_HEAP_SIZE] = { 0 };
#else
void *
malloc_func(
                       ...
                       uint8 **p_buffer, uint32 *p_size)
{
    ...
    *p_buffer = (uint8_t *)bh_read_file_to_buffer(wasm_file_name, p_size);
    ...
    return *p_buffer != nullptr;
}

void
module_destroyer(uint8 *buffer, uint32 size)
{
    if (!buffer) {
        return;
    }
...
main()
{
    ...
    /* Process options. */
    for (__argc--, __argv++; __argc > 0 && __argv[0][0] == '-';
         __argc--, __argv++) {
        if (!strcmp(__argv[0], "-f") || !strcmp(__argv[0], "--function")) {
            __argc--, __argv++;
            if (__argc < 2) {
                return print_help();
            }
...
#if WASM_ENABLE_INTERP != 0
        else if (!strcmp(__argv[0], "--interp")) {
            running_mode = Mode_Interp;
        }
#endif
#if WASM_ENABLE_FAST_JIT != 0
        else if (!strcmp(__argv[0], "--fast-jit")) {
            running_mode = Mode_Fast_JIT;
        }
#endif
#if WASM_ENABLE_JIT != 0
        else if (!strcmp(__argv[0], "--llvm-jit")) {
            running_mode = Mode_LLVM_JIT;
        }
#endif
#if WASM_ENABLE_JIT != 0 && WASM_ENABLE_FAST_JIT != 0
        else if (!strcmp(__argv[0], "--multi-tier-jit")) {
            running_mode = Mode_Multi_Tier_JIT;
        }
#endif
#if WASM_ENABLE_LOG != 0
        else if (!strncmp(__argv[0], "-v=", 3)) {
            log_verbose_level = atoi(__argv[0] + 3);
            if (log_verbose_level < 0 || log_verbose_level > 5)
                return print_help();
        }
#endif
        else if (!strcmp(__argv[0], "--repl")) {
            is_repl_mode = true;
        }
        else if (!strncmp(__argv[0], "--stack-size=", 13)) {
            if (__argv[0][13] == '\0')
                return print_help();
            stack_size = atoi(__argv[0] + 13);
        }
        else if (!strncmp(__argv[0], "--heap-size=", 12)) {
            if (__argv[0][12] == '\0')
                return print_help();
            heap_size = atoi(__argv[0] + 12);
        }
#if WASM_ENABLE_JIT != 0
        else if (!strncmp(__argv[0], "--llvm-jit-size-level=", 22)) {
            if (__argv[0][22] == '\0')
                return print_help();
            llvm_jit_size_level = atoi(__argv[0] + 22);
        ...
        else if (!strncmp(__argv[0], "--llvm-jit-opt-level=", 21)) {
            if (__argv[0][21] == '\0')
                return print_help();
            llvm_jit_opt_level = atoi(__argv[0] + 21);
...
#endif
#if WASM_ENABLE_MULTI_MODULE != 0
        else if (!strncmp(__argv[0], MODULE_PATH, strlen(MODULE_PATH))) {
            module_search_path = handle_module_path(__argv[0]);
...
#endif
#if WASM_ENABLE_LIB_PTHREAD != 0 || WASM_ENABLE_LIB_WASI_THREADS != 0
        else if (!strncmp(__argv[0], "--max-threads=", 14)) {
            if (__argv[0][14] == '\0')
                return print_help();
            wasm_runtime_set_max_thread_num(atoi(__argv[0] + 14));
        }
#endif
#if WASM_ENABLE_DEBUG_INTERP != 0
        else if (!strncmp(__argv[0], "-g=", 3)) {
            char *port_str = strchr(__argv[0] + 3, ':');
            char *port_end;
            if (port_str == nullptr)
                return print_help();
            *port_str = '\0';
...
#endif
        else if (!strncmp(__argv[0], "--version", 9)) {
            ...
            libc_wasi_parse_result_t result =
                libc_wasi_parse(__argv[0], &wasi_parse_ctx);
            switch (result) {
    ...
    if (wasm_runtime_is_xip_file(wasm_file_buf, wasm_file_size)) {
        ...
        if (!(wasm_file_mapped = static_cast<uint8 *>(
                  os_mmap(nullptr, static_cast<uint32>(wasm_file_size),
                          map_prot, map_flags)))) {
...
#if WASM_ENABLE_MULTI_MODULE != 0
    wasm_runtime_set_module_reader(module_reader_callback, module_destroyer);
#endif
...
#if WASM_ENABLE_LIBC_WASI != 0
    libc_wasi_init(wasm_module, __argc, __argv, &wasi_parse_ctx);
#endif
    ...
    if (is_repl_mode) {
        ...
        exception = static_cast<const char *>(
        ...
        exception =
            static_cast<const char *>(app_instance_main(wasm_module_inst));
        if (exception) {
            /* got an exception */
            ret = 1;
        }
